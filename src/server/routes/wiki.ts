...
export function createWikiRouter(wikiRoot: string) {
  const wiki = new Hono()

  wiki.get('/files', async (c) => {
    const files = await walkDir(wikiRoot)
  ...
  wiki.get('/files/*', async (c) => {
    ...
    const fullPath = join(wikiRoot, path)
    try {
      const content = await readFile(fullPath, 'utf-8')
  ...
  wiki.post('/files', async (c) => {
    ...
    const fullPath = join(wikiRoot, file.path)
...
const wiki = createWikiRouter(process.env.WIKI_ROOT ?? '/home/ubuntu/wiki')
export default wiki
